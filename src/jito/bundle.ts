...
const TIP_CACHE_TTL = 3_000; // 3s: balance between freshness and RPS budget (Jito limit 10 RPS)
...
export async function resolveTipLamports(tipMultiplier = 1.0, urgent = false): Promise<number> {
  ...
  // Dynamic floor: при тихой сети (Jito p50 < threshold) снижаем пол до
  // lowActivityFloorSol. p50 подтверждает, что такого tip'а хватает для landing.
  const lowActivityCfg = config.jito as any;
  const lowActivityFloor = Math.floor((lowActivityCfg.lowActivityFloorSol ?? config.jito.minTipAmountSol) * 1e9);
  const p50Threshold = Math.floor((lowActivityCfg.lowActivityP50ThresholdSol ?? 0) * 1e9);
  const isLowActivity = p50Threshold > 0 && floor.p50 > 0 && floor.p50 < p50Threshold;
  const effectiveMinTip = isLowActivity ? lowActivityFloor : Math.floor(config.jito.minTipAmountSol * 1e9);

  const dynamicTip = urgent ? floor.p95 : floor.p75;
  const baseTip = Math.max(dynamicTip, staticTip, effectiveMinTip);
  const finalTip = Math.min(Math.floor(baseTip * tipMultiplier), maxTip);

  logger.debug(`Jito tip: ${(finalTip / 1e9).toFixed(6)} SOL (dynamic=${(dynamicTip / 1e9).toFixed(6)}, static=${(staticTip / 1e9).toFixed(6)}, floor=${(effectiveMinTip / 1e9).toFixed(6)}${isLowActivity ? ' [LOW-ACTIVITY]' : ''}, urgent=${urgent}, multiplier=${tipMultiplier.toFixed(2)})`);
  return finalTip;
}
...
export async function sendJitoBurst(
  ...
  // Pre-warm tip floor cache so parallel sends don't all fetch independently
  await getTipFloor();
