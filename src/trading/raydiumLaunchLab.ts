...
export class PoolMigratedError extends Error {
  constructor(public status: number, public migrateType: number) {
    super(`LaunchLab pool migrated (status=${status}), use ${migrateType === 1 ? 'CPMM' : 'AMM v4'}`);
...
export async function resolveLaunchLabPool(
  ...
  if (pool.status !== 0 && !allowMigrated) {
    throw new PoolMigratedError(pool.status, pool.migrateType);
  }
...
export async function buyTokenLaunchLab(
  ...
  logEvent('TX_DIAGNOSTIC', {
    ...
    action: 'buy',
    mint: mintA.toBase58(),
    disc: 'LAUNCH_BUY_EXACT_IN',
    poolId: poolId.toBase58(),
    auth: getLaunchAuthPDA().toBase58(),
    configId: pool.configId.toBase58(),
    platformId: pool.platformId.toBase58(),
    ...
    virtualA: pool.virtualA.toString(),
    virtualB: pool.virtualB.toString(),
    solIn: solIn.toString(),
    expectedOut: expectedOut.toString(),
    minOut: minOut.toString(),
    status: pool.status,
    migrateType: pool.migrateType,
  }, { mint: mintA.toBase58(), protocol: 'raydium-launchlab' });
