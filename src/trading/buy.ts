...
export function buildBuyInstructionFromCreate(
  ...
  const expectedTokensGross = (solAmountLamports * virtualTokenReserves)
    / (virtualSolReserves + solAmountLamports);

  if (expectedTokensGross === 0n) {
    throw new Error('buildBuyInstructionFromCreate: expectedTokens is 0 (amountSol too small?)');
  }

  // EV-FIX: Pump.fun charges ~1% fee on tokens (virtual fee deducted by the program).
  // Without this adjustment, our slippage buffer is 1% smaller than intended.
  // At 1500bps slippage, real buffer was only 500bps — causing unnecessary tx failures.
  const expectedTokens = expectedTokensGross * 99n / 100n;

  // min_tokens_out: защита от проскальзывания цены (now applied AFTER fee deduction)
...
export async function buyToken(
  ...
  logEvent('TX_DIAGNOSTIC', {
    ...
    action: 'buy',
    mint: mint.toBase58(),
    disc: 'BUY_EXACT_SOL_IN',
    tokenProgramId: mintState.tokenProgramId.toBase58(),
    ata: tokenATA.toBase58(),
    bondingCurve: getBondingCurvePDA(mint).toBase58(),
    bondingCurveV2: getBondingCurveV2PDA(mint).toBase58(),
    vault: getVaultPDA(getBondingCurvePDA(mint), mint, mintState.tokenProgramId).toBase58(),
    creatorVault: getCreatorVaultPDA(creator).toBase58(),
    feeRecipient: feeRecipient.toBase58(),
    solAmount: solAmount.toString(),
    minTokensOut: minTokensOut.toString(),
    isMayhem,
  }, { mint: mint.toBase58(), protocol: 'pump.fun' });
